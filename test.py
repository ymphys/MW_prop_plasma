...
# 时间轴（单位：纳秒）
time_axis = np.arange(n_steps) * dt * 1e9  # 转换成 ns

plt.figure(figsize=(8,3))
plt.plot(time_axis, src_record, label="Source (src_val)")
plt.xlabel("Time (ns)")
plt.ylabel("Ez source value (V/m)")
plt.title("Injected source waveform")
plt.grid(True)
plt.legend()
plt.show()
plt.savefig("Injected_source_waveform.jpeg")

# Plot probe time series
plt.figure(figsize=(8,3))
time_axis = (np.arange(n_steps) * dt) * 1e9  # ns
plt.plot(time_axis, Ez_probe)
plt.xlabel("Time (ns)")
plt.ylabel("Ez at probe (V/m)")
plt.title("Probe signal")
plt.grid(True)
plt.show()
plt.savefig("Probe_signal.jpeg")
